...
def ircad_preparation(datadirpath, output_datadirpath="output_data", organ="liver",res='100'):
    ...
    #     print scan.shape
    #     name = folder.split('/')[-2]
    #     DW.write(scan, output_datadirpath + '/' +str(name) +'_' + str(res)+'.vtk', metadata={"voxelsize_mm": [1, 1, 1]})
    #     seznam[int(name)] = output_datadirpath + '/' +str(name) +'_' + str(res)+'.vtk'
    #     break
    ...
    f = h5py.File(output_datadirpath+'/data.hdf5', 'a')
    for i in seznam:
...
def main():
    ...
    parser.add_argument('-r', '--resolution', default=100)
